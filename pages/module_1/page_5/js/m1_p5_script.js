...
function disableAll() {
  playClickThen();
  $("#home,#music,#info,.wrapTextaudio,#full-screen").prop("disabled", true);
...
function enableAll() {
  playClickThen();
  $("#home,#music,#info,.wrapTextaudio,#full-screen").prop("disabled", false);
  const audio = document.getElementById("audio_src");
  if (_controller._globalMusicPlaying) {
    audio.muted = false;
    audio.play();
...
function togglePop1Audio(el, src) {
  ...
  if (audio.src !== el.baseURI + src) {
    ...
    audio.muted = false;
    audio.play().catch(() => { });
    // Immediately update class for first click
...
function replayLastAudio(btnElement) {
  ...
  if (!audio.paused && !audio.ended) {
    // Audio is currently playing, just toggle mute
  ...
  // Audio is paused or ended, play from start
  btnElement.classList.remove("paused");
  btnElement.classList.add("playing");
  audio.muted = false;
...
// Tablet landscape
  ...
  widths = [70, 30, 40, 60, 50, 55];

// Desktop
} else {
  widths = [90, 40, 50, 80, 60, 70];
}

var imgWidth = widths[num - 1];
...
$img.css({
  ...
  var btnOffset = parentBtn.offset();
  var startLeft = btnOffset.left + parentBtn.width() / 2 - imgWidth / 2;
  var startTop = btnOffset.top + parentBtn.height() / 2 - imgWidth / 2;
  $img.css({ left: startLeft, top: startTop });
  ...
  if (clickCount === 1) {
    playBtnSounds(_pageData.sections[sectionCnt - 1].content.ostAudios[_btnClicked]);
    clearText();
    setTimeout(function () { $(".bar-holding").css("transform", "rotate(-5deg)"); }, 500);
  }
  ...
  if (clickCount === 2) {
    _globalCicked++;
    setTimeout(function () { evaluateSeesaw(); }, 200);
    playBtnSounds(_pageData.sections[sectionCnt - 1].content.ostAudios[_btnClicked]);
    clearText();
    audioEnd(function () { checkHeavyLight(leftWeight, rightWeight); });
  }

  var targetOffset = $target.offset();
  var targetLeft = targetOffset.left + $target.width() / 2 - imgWidth / 2;
  var targetTop = targetOffset.top + $target.height() / 2 - imgWidth / 2;

  $img.animate(
    { left: targetLeft, top: targetTop },
    300,
    function() {
      $img.appendTo($target).css({
        position: "relative",
        left: 0,
        top: 0,
        width: imgWidth + "px",
